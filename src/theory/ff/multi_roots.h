 ...
 */
std::vector<Poly> findZero(Ideal& ideal,
                           const Env& env,
...
class AssignmentEnumerator
{
 public:
   ...
   */
  virtual std::optional<Poly> next() = 0;
  /**
...
{
 public:
  ListEnumerator(std::vector<Poly>&& options);
  ~ListEnumerator() override;
  std::optional<Poly> next() override;
  std::string name() override;

 private:
  std::vector<Poly> d_remainingOptions;
};
...
{
 public:
  RoundRobinEnumerator(const std::vector<Poly>& vars, const SingularRing& ring);
  ~RoundRobinEnumerator() override;
  std::optional<Poly> next() override;
  std::string name() override;

 private:
  const std::vector<Poly> d_vars;
